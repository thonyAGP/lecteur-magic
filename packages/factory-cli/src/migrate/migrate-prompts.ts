...
export const buildTestUiPrompt = (
  ...
  const parts: string[] = [
    ...
    '',
    '⚠️ CRITICAL ZUSTAND MOCK PATTERN (MANDATORY):',
    '',
    ...
    'vi.mocked(useMyStore.getState).mockReturnValue({',
    '  items: [],',
    '  isLoading: false,',
    '  loadItems: vi.fn()',
    '})',
    '```',
    '',
    '❌ WRONG Pattern (DO NOT USE - causes mockReturnValue error):',
    '```typescript',
    'vi.mock("@/stores/myStore", () => ({',
    '  useMyStore: vi.fn(() => ({ ... })),  // ❌ BAD',
    '  useMyStore: { setState: vi.fn() }     // ❌ DUPLICATE KEY',
    '}))',
    '```',
    '',
    'REFERENCE: See adh-web/src/__tests__/sessionListStore.test.ts lines 63-67',
    '',
