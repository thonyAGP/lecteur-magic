# ADH IDE 90 - Edition Facture Tva(Compta&Ve)

> **Analyse**: 2026-02-07 14:13
> **Pipeline**: V7.2 Enrichi

## RESUME EXECUTIF

- **Fonction**: Edition Facture Tva(Compta&Ve)
- **Tables modifiees**: 0
- **Complexite**: **BASSE** (0/100)
- **Statut**: NON_ORPHELIN
- **Raison**: Appele par 3 programme(s): Factures (Tble Compta&Vent (IDE 89), Factures_Check_Out (IDE 54), Lancement Edition Facture (IDE 279)

## PROGRAMMES LIES

| Direction | Programmes |
|-----------|------------|
| **Appele par** | [Factures (Tble Compta&Vent (IDE 89)](ADH-IDE-89.md), [Factures_Check_Out (IDE 54)](ADH-IDE-54.md), [Lancement Edition Facture (IDE 279)](ADH-IDE-279.md) |
| **Appelle** | (aucun) |

## STATISTIQUES

| Metrique | Valeur |
|----------|--------|
| Taches | 4 |
| Lignes Logic | 138 |
| Expressions | 4 |
| Tables | 8 |

---
*Spec SUMMARY generee par Pipeline V7.2*
