# ADH IDE 65 - Edition & Mail Easy Check Out

> **Analyse**: 2026-02-07 13:36
> **Pipeline**: V7.2 Enrichi

## RESUME EXECUTIF

- **Fonction**: Edition & Mail Easy Check Out
- **Tables modifiees**: 1
- **Complexite**: **BASSE** (7/100)
- **Statut**: NON_ORPHELIN
- **Raison**: Appele par 4 programme(s): Récap Trait Easy Check-Out (IDE 56), Solde Easy Check Out (IDE 64), Reedition Recap Easy Check Out (IDE 67), Solde Easy Check Out (IDE 287)

## PROGRAMMES LIES

| Direction | Programmes |
|-----------|------------|
| **Appele par** | [Récap Trait Easy Check-Out (IDE 56)](ADH-IDE-56.md), [Solde Easy Check Out (IDE 64)](ADH-IDE-64.md), [Reedition Recap Easy Check Out (IDE 67)](ADH-IDE-67.md), [Solde Easy Check Out (IDE 287)](ADH-IDE-287.md) |
| **Appelle** | (aucun) |

## STATISTIQUES

| Metrique | Valeur |
|----------|--------|
| Taches | 4 |
| Lignes Logic | 116 |
| Expressions | 4 |
| Tables | 7 |

---
*Spec SUMMARY generee par Pipeline V7.2*
