# PVE IDE 283 - GetPrinter/t impression

> **Analyse**: 2026-02-03 19:24
> **Pipeline**: V7.2 Enrichi

## RESUME EXECUTIF

- **Fonction**: GetPrinter/t impression
- **Tables modifiees**: 0
- **Complexite**: **BASSE** (0/100)
- **Statut**: NON_ORPHELIN
- **Raison**: Appele par 7 programme(s): Edition ticket (Tva) LEX (IDE 205), Edition ticket (Tva) (IDE 206), Edition Recap Rendez-vous (IDE 207), Edition ticket (IDE 273), Edition ticket (Tva) (IDE 280), Edition ticket (Tva) (IDE 362), Edition ticket (Tva) (IDE 442)

## PROGRAMMES LIES

| Direction | Programmes |
|-----------|------------|
| **Appele par** | [Edition ticket (Tva) LEX (IDE 205)](PVE-IDE-205.md), [Edition ticket (Tva) (IDE 206)](PVE-IDE-206.md), [Edition Recap Rendez-vous (IDE 207)](PVE-IDE-207.md), [Edition ticket (IDE 273)](PVE-IDE-273.md), [Edition ticket (Tva) (IDE 280)](PVE-IDE-280.md), [Edition ticket (Tva) (IDE 362)](PVE-IDE-362.md), [Edition ticket (Tva) (IDE 442)](PVE-IDE-442.md) |
| **Appelle** | (aucun) |

## STATISTIQUES

| Metrique | Valeur |
|----------|--------|
| Taches | 3 |
| Lignes Logic | 25 |
| Expressions | 6 |
| Tables |  |

---
*Spec SUMMARY generee par Pipeline V7.2*
