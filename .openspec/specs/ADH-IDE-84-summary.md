# ADH IDE 84 - SP Caractères Interdits

> **Analyse**: 2026-02-07 14:03
> **Pipeline**: V7.2 Enrichi

## RESUME EXECUTIF

- **Fonction**: SP Caractères Interdits
- **Tables modifiees**: 0
- **Complexite**: **BASSE** (0/100)
- **Statut**: NON_ORPHELIN
- **Raison**: Appele par 7 programme(s): Transaction Nouv vente avec GP (IDE 237), Transaction Nouv vente PMS-584 (IDE 238), Transaction Nouv vente PMS-721 (IDE 239), Transaction Nouv vente PMS-710 (IDE 240), Saisie transaction 154  N.U (IDE 307), Saisie transaction Nouv vente (IDE 310), Saisie transaction Nouv vente (IDE 316)

## PROGRAMMES LIES

| Direction | Programmes |
|-----------|------------|
| **Appele par** | [Transaction Nouv vente avec GP (IDE 237)](ADH-IDE-237.md), [Transaction Nouv vente PMS-584 (IDE 238)](ADH-IDE-238.md), [Transaction Nouv vente PMS-721 (IDE 239)](ADH-IDE-239.md), [Transaction Nouv vente PMS-710 (IDE 240)](ADH-IDE-240.md), [Saisie transaction 154  N.U (IDE 307)](ADH-IDE-307.md), [Saisie transaction Nouv vente (IDE 310)](ADH-IDE-310.md), [Saisie transaction Nouv vente (IDE 316)](ADH-IDE-316.md) |
| **Appelle** | (aucun) |

## STATISTIQUES

| Metrique | Valeur |
|----------|--------|
| Taches | 2 |
| Lignes Logic | 19 |
| Expressions | 2 |
| Tables | 0 |

---
*Spec SUMMARY generee par Pipeline V7.2*
