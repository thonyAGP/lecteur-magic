# ADH IDE 233 - Appel Print ticket vente PMS28

> **Analyse**: 2026-01-30 19:09
> **Pipeline**: V7.2 Enrichi

## RESUME EXECUTIF

- **Fonction**: Appel Print ticket vente PMS28
- **Tables modifiees**: 0
- **Complexite**: **BASSE** (5/100)
- **Statut**: NON_ORPHELIN
- **Raison**: Appele par 11 programme(s): Historique des ventes - Gratui (IDE 312), Annulation Ventes Gratuites (IDE 319), Histo ventes payantes /PMS-623 (IDE 245), Transaction Nouv vente avec GP (IDE 237), Transaction Nouv vente PMS-721 (IDE 239), Transaction Nouv vente PMS-710 (IDE 240), Histo ventes payantes (IDE 243), Histo ventes payantes /PMS-605 (IDE 244), Histo ventes IGR (IDE 252), Histo ventes Gratuités (IDE 253), VAD validés à imprimer (IDE 255)

## PROGRAMMES LIES

| Direction | Programmes |
|-----------|------------|
| **Appele par** | [Historique des ventes - Gratui (IDE 312)](ADH-IDE-312.md), [Annulation Ventes Gratuites (IDE 319)](ADH-IDE-319.md), [Histo ventes payantes /PMS-623 (IDE 245)](ADH-IDE-245.md), [Transaction Nouv vente avec GP (IDE 237)](ADH-IDE-237.md), [Transaction Nouv vente PMS-721 (IDE 239)](ADH-IDE-239.md), [Transaction Nouv vente PMS-710 (IDE 240)](ADH-IDE-240.md), [Histo ventes payantes (IDE 243)](ADH-IDE-243.md), [Histo ventes payantes /PMS-605 (IDE 244)](ADH-IDE-244.md), [Histo ventes IGR (IDE 252)](ADH-IDE-252.md), [Histo ventes Gratuités (IDE 253)](ADH-IDE-253.md), [VAD validés à imprimer (IDE 255)](ADH-IDE-255.md) |
| **Appelle** | [ Print ticket vente (IDE 234)](ADH-IDE-234.md), [ Print ticket vente LEX (IDE 235)](ADH-IDE-235.md) |

## STATISTIQUES

| Metrique | Valeur |
|----------|--------|
| Taches | 1 |
| Lignes Logic | 44 |
| Expressions | 2 |
| Tables | 0 |

---
*Spec SUMMARY generee par Pipeline V7.2*
