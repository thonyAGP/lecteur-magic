# ADH IDE 50 -   Initialistaion Easy Arrival

> **Version spec** : 2.1 (Enhanced)
> **Genere le** : 2026-01-27
> **Source** : `D:\Data\Migration\XPA\PMS\ADH\Source\Prg_50.xml`

---

---

<!-- TAB:Fonctionnel -->

## 1. IDENTIFICATION

| Attribut | Valeur |
|----------|--------|
| **Format IDE** | ADH IDE 50 |
| **Fichier XML** | Prg_50.xml |
| **Description** |   Initialistaion Easy Arrival |
| **Type** | B (O=Online, B=Batch) |
| **Parametres** | 0 |
| **Module** | ADH |
| **Dossier IDE** | Divers |

> **Note**: Ce programme est Prg_50.xml. L'ID XML (50) peut differer de la position IDE (50).


---

## PARTIE I: SPECIFICATION FONCTIONNELLE (Annotations)

### 1.1 Objectif Metier
> A completer dans `.openspec/annotations/ADH-IDE-50.yaml`
### 1.2 Flux Utilisateur
> A completer dans annotations YAML

### 1.3 Notes Migration
> A completer dans annotations YAML

### 1.4 Dependances ECF



### 1.5 Tags
> Aucun tag defini

---

---

<!-- TAB:Technique -->

## 2. TABLES (1 tables - 0 en ecriture)

| IDE# | Nom Physique | Nom Logique | Access | Usage |
|------|--------------|-------------|--------|-------|
| #881 | `evenement_temp` | evenement_temp | R | 1x |

---

## 3. PARAMETRES D'ENTREE (0)

| # | Nom | Type | Description |
|---|-----|------|-------------|

---

## 4. VARIABLES PRINCIPALES

### 4.1 Variables de travail (W0/V0)

| Nom | Type | Role |
|-----|------|------|

### 4.2 Variables globales (VG)

| Variable | Role |
|----------|------|
| VG.LOGIN | - |
| VG.USER | - |
| VG.Retour Chariot | - |
| VG.DROIT ACCES IT ? | - |
| VG.DROIT ACCES CAISSE ? | - |
| VG.BRAZIL DATACATCHING? | - |
| VG.USE MDR | - |
| VG.VRL ACTIF ? | - |
| VG.ECI ACTIF ? | - |
| VG.COMPTE CASH ACTIF ? | - |
| VG.IND SEJ PAYE ACTIF ? | - |
| VG.CODE LANGUE USER | - |
| VG.EFFECTIF ACTIF ? | - |
| VG.TAXE SEJOUR ACTIF ? | - |
| VG.N° version | - |

> Total: 118 variables mappees

---

## 5. EXPRESSIONS (6 total, 3 decodees)

| # | Expression brute | Decode |
|---|------------------|--------|
| 1 | `'LOCATION'` | `'LOCATION'` |
| 2 | `'COURS'` | `'COURS'` |
| 3 | `'ENFANT'` | `'ENFANT'` |
| 4 | `Trim({0,2})` | `Trim({0,2})` |
| 5 | `Trim({0,4})` | `Trim({0,4})` |
| 6 | `Trim({0,6})` | `Trim({0,6})` |

---

## 6. STATISTIQUES

| Metrique | Valeur |
|----------|--------|
| Tables | 1 (0 W /  R) |
| Parametres | 0 |
| Variables locales | 0 |
| Expressions | 6 |
| Expressions 100% decodees | 3 (50%) |

---

## 7. HISTORIQUE

| Date | Action | Auteur |
|------|--------|--------|
| 2026-01-27 | Creation specification v2.0 | Claude |

---

*Specification v2.0 - Generee automatiquement par Generate-ProgramSpecV2.ps1*

---

<!-- TAB:Cartographie -->

## CARTOGRAPHIE

*Aucun callee identifie - programme terminal ou appels dynamiques*

### Metriques

| Metrique | Valeur |
|----------|--------|
| Tables | 1 |
| Expressions | 6 |
| Complexite | Faible |

---

*Spec V2.1 avec marqueurs TAB - Genere automatiquement*
