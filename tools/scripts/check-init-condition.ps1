...
Write-Host "=== Record Prefix/Suffix pour $Project IDE $PrgId TaskIsn=$TaskIsn ===" -ForegroundColor Cyan

foreach ($lu in $task.TaskLogic.LogicUnit) {
    $level = $lu.Level.val
    $type = $lu.Type.val
    
    if ($level -eq "R" -and ($type -eq "P" -or $type -eq "S")) {
        ...
        foreach ($line in $lu.LogicLines.LogicLine) {
            ...
            if ($line.BLOCK) {
                ...
                Write-Host "  L$lineNum : BLOCK IF (Exp=$condExp)"
            }
            elseif ($line.END_BLK) {
                Write-Host "  L$lineNum : END_BLK"
            }
            elseif ($line.Update) {
                $fieldId = $line.Update.FieldID.val
                $withValue = $line.Update.WithValue.val
                Write-Host "  L$lineNum : Update FieldID=$fieldId WithValue=$withValue"
            }
        }
    }
}
