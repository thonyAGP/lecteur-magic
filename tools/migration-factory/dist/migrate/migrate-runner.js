...
export const runMigration = async (programIds, batchId, batchName, config) => {
    ...
    for (const chunk of chunks) {
        ...
        for (const r of results) {
            ...
            if (r.analysis)
                analyses.set(r.result.programId, r.analysis);
        }
    }
...
// ─── Run single phase ──────────────────────────────────────────
export const runSinglePhase = async (phase, programIds, config) => {
        ...
        try {
            switch (phase) {
                ...
                case MP.TESTS_UI: {
                    ...
                    if (phase === MP.TYPES)
                        await runTypesPhase(programId, analysis, config);
                    else if (phase === MP.STORE)
                        await runStorePhase(programId, analysis, config);
                    else if (phase === MP.API)
                        await runApiPhase(programId, analysis, config);
                    else if (phase === MP.PAGE)
                        await runPagePhase(programId, analysis, config);
                    else if (phase === MP.COMPONENTS)
                        await runComponentsPhase(programId, analysis, config);
                    else if (phase === MP.TESTS_UNIT)
                        await runTestsUnitPhase(programId, analysis, config);
                    else if (phase === MP.TESTS_UI)
                        await runTestsUiPhase(programId, analysis, config);
                    break;
                }
                case MP.VERIFY_TSC:
                case MP.FIX_TSC:
                ...
                case MP.REVIEW: {
                    ...
                    if (fs.existsSync(aFile)) {
                        const analysis = JSON.parse(fs.readFileSync(aFile, 'utf8'));
...
export const getMigrateStatus = (trackerFile) => {
...
// ─── Batch creation ────────────────────────────────────────────
export const createBatch = (batchId, batchName, programIds, config) => {
    const trackerFile = path.join(config.migrationDir, config.contractSubDir, 'tracker.json');
...
// ─── Helpers ───────────────────────────────────────────────────
const emit = (config, type, message, extra) => {
    if (!config.onEvent)
        return;
    config.onEvent({
        type,
...
const saveMigrateTracker = (trackerFile, data) => {
    try {
