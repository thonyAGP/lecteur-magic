...
export const buildReport = (input) => {
    const { projectName, programs, programStatuses, sharedPrograms, sccs, maxLevel, modulesOutput, readiness, decommission, tracker, } = input;
    ...
    if (useBatchesAsModules) {
        ...
        const priorityMap = new Map(priorityResult.prioritizedModules.map(p => [p.root, p]));
...
export const buildMultiProjectReport = (input) => {
    const entries = input.projects.map(p => {
        if (p.reportInput) {
