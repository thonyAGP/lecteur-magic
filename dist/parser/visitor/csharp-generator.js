...
exports.generateCSharp = generateCSharp;
 ...
 */
function generateCaseExpression(args) {
    if (args.length < 4) {
...
class CSharpGenerator {
    ...
    visitSpecialReference(node) {
        ...
        switch (refType) {
            case tokens_1.SpecialRefType.PROG:
                return `Programs[${id}]`;
            case tokens_1.SpecialRefType.DSOURCE:
                return `Tables[${comp}][${id}]`;
            case tokens_1.SpecialRefType.VAR:
                if (comp === 0) {
                    return `${this.options.fieldPrefix}.V${id}`;
                }
                return `GetVariable(${comp}, ${id})`;
            case tokens_1.SpecialRefType.FORM:
                return `Forms[${id}]`;
            default:
    ...
    visitBinaryExpression(node) {
        const left = (0, visitor_1.visit)(node.left, this);
        const right = (0, visitor_1.visit)(node.right, this);
        const operatorMap = {
            [ast_1.BinaryOperator.Add]: '+',
            [ast_1.BinaryOperator.Subtract]: '-',
            [ast_1.BinaryOperator.Multiply]: '*',
            [ast_1.BinaryOperator.Divide]: '/',
            [ast_1.BinaryOperator.Power]: '/* ^ */ Math.Pow',
            [ast_1.BinaryOperator.Mod]: '%',
            [ast_1.BinaryOperator.Concat]: '+',
            [ast_1.BinaryOperator.Equal]: '==',
            [ast_1.BinaryOperator.NotEqual]: '!=',
            [ast_1.BinaryOperator.LessThan]: '<',
            [ast_1.BinaryOperator.GreaterThan]: '>',
            [ast_1.BinaryOperator.LessEqual]: '<=',
            [ast_1.BinaryOperator.GreaterEqual]: '>=',
            [ast_1.BinaryOperator.And]: '&&',
            [ast_1.BinaryOperator.Or]: '||',
        };
        // Special case for power operator
        if (node.operator === ast_1.BinaryOperator.Power) {
    ...
    visitUnaryExpression(node) {
        const operand = (0, visitor_1.visit)(node.operand, this);
        switch (node.operator) {
            case ast_1.UnaryOperator.Negate:
                return `(-${operand})`;
            case ast_1.UnaryOperator.Not:
                return `(!${operand})`;
            default:
    ...
    visitFunctionCall(node) {
        const args = node.arguments.map((arg) => (0, visitor_1.visit)(arg, this));
 ...
 */
function generateCSharp(ast, options) {
    const generator = new CSharpGenerator(options);
    return (0, visitor_1.visit)(ast, generator);
}
//# sourceMappingURL=csharp-generator.js.map